!**<main program>*******************************************************
!      PROGRAM QUASI_STEXP_2D
!     -----------------------
!
!-----------------------------------------------------------------------
!  This program tries to estimate the parameters of an elstic peak and 
!  a single "stretched exponential" pertaining to some relevant data.
!  It was modified from QUASI_LINES_2D, a 2-D Bayesian Quasi-elastic 
!  line-fitting program. The resolution function is given on a constant 
!  X-binning grid, and is assumed to be invariant. The background is 
!  assumed to be linear and the data should be of an intermediate Genie
!  binary format, such as that output by batch-mode ICON.
!-----------------------------------------------------------------------
!  Written by: D.S. Sivia, Rutherford Appleton Lab., Oxfordshire, England.
!-----------------------------------------------------------------------
! Initial release for trials by CJC, MAA, WSH.        (DSS: 27-FEB-1992)
! Changed file access from sequential to direct.      (DSS:  5-MAR-1992)
! Changed STEPSZ reduction from 0.85 to 0.6 .         (DSS:  7-MAR-1992)
! Can read in a detector normalisation file.          (DSS: 12-MAR-1992)
! Readonly for resol. file; silent-running modified.  (DSS: 14-MAY-1992)
! WBF array-size increased in subroutine INBUFF.      (DSS: 28-MAY-1992)
! Fixed bug in CLRBUF; problem for lots of detectors. (DSS: 29-MAY-1992)
! Hard-wired meV as input and ueV as output.          (DSS:  2-OCT-1992)
! Put in STEPSZ safety-valve in case Chisq stuck!     (DSS: 27-OCT-1992)
! Took out small and negative number safety-valve.    (DSS: 17-DEC-1992)
! Fixed bug in DATIN1 (no more large error-bars!).    (DSS: 27-SEP-1993)
! Modification of QUASI_LINES_2D.FOR .                (DSS: 26-APR-1994)
! Do "stretched-exp" quasi_lines properly, I hope!    (DSS:  9-JUN-1999)
! Corrected minor crashable bug!                      (DSS:  7-JUL-1999)
!-----------------------------------------------------------------------
!
SUBROUTINE QLstexp(numb,x_in,y_in,e_in,reals,opft, &
 XD_in,XB_in,YB_in,Wy_in,We_in,dtn,xsc,sfile,rfile,l_fn, &
 nd_out,xout,yout,eout,yfit,yprob)
...
!f2py intent(in) :: x_in, y_in, e_in                    !sample data
real reals(4)
!f2py intent(in) :: reals                               !real parameters
real XD_in(m_d), XB_in(m_d), YB_in(m_d)
!f2py intent(in) :: XD_in, XB_in, YB_in                 !sample xrange, res data (blur)
real Wy_in(m_sp), We_in(m_sp), dtn(m_sp), xsc(m_sp)
!f2py intent(in) :: Wy_in, We_in, dtn, xsc              !fixed width data
integer numb(9)
!f2py intent(in) :: numb                                !integer parameters
integer opft(4)
!f2py intent(in) :: opft                                 !options parameters
integer l_fn
!f2py intent(in) :: l_fn                                 !length of filenames
character*140 sfile, rfile
!f2py intent(in) :: sfile, rfile                         !sample & res filenames
integer nd_out
!f2py intent(out) :: nd_out                              !number of output points
real xout(m_d), yout(m_d), eout(m_d)
!f2py intent(out) :: xout, yout, eout                    !data values
real yfit(4*m_d)
!f2py intent(out) :: yfit                                !fit values
real yprob(2)
!f2py intent(out) :: yprob                               !probability values
